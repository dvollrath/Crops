...
local max = r(max)
forvalues x = 2(1)`max' {
	// There is an issue with Conley SE for North Africa - override cutoff
	local cuthold=$cutoff
	if `x'==10 { // for North Africa
		global cutoff 300 // set to this cutoff
	}
	est_reg if jv_subregion==`x'
	estimates store reg_subspatial_`x'
	global cutoff=`cuthold' // restore global cutoff 
}
