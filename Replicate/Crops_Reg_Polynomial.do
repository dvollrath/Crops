...
//////////////////////////////////////
gen urb_perc_2000_sq = urb_perc_2000^2
gen ln_light_mean_sq = ln_light_mean^2
gen ln_popc_2000_sq = ln_popc_2000^2
gen x_urb_light = urb_perc_2000*ln_light_mean
gen x_urb_pop = urb_perc_2000*ln_popc_2000
gen x_light_pop = ln_light_mean*ln_popc_2000
...
local controls urb_perc_2000 ln_light_mean ln_popc_2000 urb_perc_2000_sq ln_light_mean_sq ln_popc_2000_sq x_urb_light x_urb_pop x_light_pop // urban percent and light mean and total population
...
estout base1 base2 harv1 harv2 kg1 kg2 using "./Drafts/tab_beta_crop_poly.tex", /// write the region results to Tex file
	replace style(tex) ///
...
graph export "./Drafts/fig_coef_crop_poly.png", replace as(png)
graph export "./Drafts/fig_coef_crop_poly.eps", replace as(eps)
...
estout urbc1 urbc2 perc1 perc2 poor1 poor2 using "./Drafts/tab_beta_crop_sub_poly.tex", /// write the region results to Tex file
	replace style(tex) ///
	cells(b(fmt(3)) se(par fmt(3))) ///
	stats(p_zero p_diff N_country N_obs r2, fmt(%9.3f %9.3f %9.0g %9.0g %9.2f) labels("p-value $\beta_g=0$" "p-value $\beta_g=\beta_{Temp}$" "Countries" "Observations" "R-square (ex. FE)")) ///
	keep(res_rurd) label mlabels(none) collabels(none) prefoot("\midrule") starlevels(* 0.10 ** 0.05 *** 0.01)
...
coefplot urbc1 || urbc2 || perc1 || perc2 || poor1 || poor2, ///
	...
	bylabels("Temperate" "Tropical" "Temperate" "Tropical" "Temperate" "Tropical")
graph export "./Drafts/fig_coef_crop_sub_poly.png", replace as(png)
graph export "./Drafts/fig_coef_crop_sub_poly.eps", replace as(eps)
