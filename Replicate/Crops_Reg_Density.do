...
foreach x of local levels { // for each province
	...
	count if state_id == `x'
	if r(N) >= 10 {
		qui reg ln_csi_yield ln_rurd_2000 `controls' if state_id==`x'
		qui replace beta = _b[ln_rurd_2000] if state_id==`x'
		qui replace se = _se[ln_rurd_2000] if state_id==`x'
	}
}

collapse (first) iso id_0 id_1 name_0 name_1 beta se jv_region jv_subregion jv_subregion_text ///
	(rawsum) rurc_2000 urbc_2000 shape_ha n (mean) suit_??? ///
	, by(state_id)
drop if beta==.
