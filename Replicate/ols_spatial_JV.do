//////////////////////////////////////
// REVISED SPATIAL SE REGRESSION
//////////////////////////////////////
/* This is a copy of the S. Hsiang scropt to do spatial AND HAC errors

We've removed the loops doing the HAC corrections as our regressions have no time dimension
This saves time on execution
The loops for spatial corrections are unchanged from the original code
...
LAST UPDATED: 4/29/2013
 ...
 -----------------------------------------------------------------------------*/
capture program drop ols_spatial_JV
program ols_spatial_JV, eclass byable(recall)
//version 9.2
version 11
...
if "`display'" == "display"{
	disp as txt " "
	disp as txt "STANDARD ERRORS UNDER OLS, WITH SPATIAL CORRECTION AND WITH SPATIAL:"
	estimates table OLS spatial, b(%7.3f) se(%7.3f) t(%7.3f) stats(N r2) 	
}

if "`star'" == "star"{
	disp as txt " "
	disp as txt "STANDARD ERRORS UNDER OLS, WITH SPATIAL CORRECTION AND WITH SPATIAL:"
	estimates table OLS spatial, b(%7.3f) star(0.10 0.05 0.01)
}
