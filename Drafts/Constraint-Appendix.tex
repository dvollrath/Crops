...
\multicolumn{8}{c}{Dependent variable is $A^{GAEZ}_{isg}$ measured by:} \\ \\
...
\begin{tabularx}{\textwidth}{lXXXXXX}
\midrule
\multicolumn{7}{c}{Dependent variable is $A^{GAEZ}_{isg}$ measured for:} \\ \\
 & Banana & Coffee & Cotton & Sugarcane & Tea & Tobacco \\
 & (1) & (2) & (3) & (4) & (5) & (6)  \\
\midrule
\input{tab_beta_cash_crop.tex}
\midrule
\end{tabularx}
}
\end{center}
\vspace{-.5cm}\singlespacing {\footnotesize Each column is the result of a regression of the (log) productivity of the specified cash crop on the labor/land ratio. Districts included in each column are those where the percent of total harvested area in the specified crop is above the 99th percentile for the percent across all districts in our dataset. All regressions include state fixed effects, a constant, and controls for the district urbanization rate, log density of district nighttime lights, log total population, log road density, share of roads of different types, distance to nearest city of 100,000 people, and a log slope index. Standard errors clustered at the state level are reported.
}
\end{table}
...
\multicolumn{7}{c}{Dependent variable is $A^{GAEZ}_{isg}$:} \\ \\
 ...
 & (1) & (2) & (3) & (4) & (5) & (6)  \\
\midrule
\input{tab_beta_terrain.tex}
\midrule
\end{tabularx}
}
\end{center}
\vspace{-.5cm}\singlespacing {\footnotesize Sets of regressions differ by exclusion of districts based on their terrain index (GAEZ). A lower terrain index indicates a more rugged terrain, so the regressions are exluding rugged areas, and including only flatter districts. State fixed effects are included, as our our standard controls for (log) night lights, (log) total population, and the urban percent. All regressions include state fixed effects, a constant, and controls for the district urbanization rate, log density of district nighttime lights, log total population, log road density, share of roads of different types, distance to nearest city of 100,000 people, and a log slope index. Conley standard errors are reported with a window of 500km.
}
\end{table}
...
\begin{table}[!htb]
\begin{center}
\caption{Estimates of $\beta$ with GAEZ constraint controls}
\label{TAB_constraints}
{\footnotesize
\begin{tabularx}{\textwidth}{lXXXXXX}
\midrule
\multicolumn{7}{c}{Dependent variable is $A^{GAEZ}_{isg}$:} \\ \\
 & \multicolumn{2}{c}{Baseline:} & \multicolumn{2}{c}{Urban pop. $<50K$:} & \multicolumn{2}{c}{Excl. N.A./Europe:} \\ \cmidrule(lr){2-3} \cmidrule(lr){4-5} \cmidrule(lr){6-7}
 & Temperate & Tropical & Temperate & Tropical & Temperate & Tropical \\
 & (1) & (2) & (3) & (4) & (5) & (6)  \\
\midrule
\input{tab_beta_robust_input_controls.tex}
\midrule
\end{tabularx}
}
\end{center}
\vspace{-.5cm}\singlespacing {\footnotesize Set of regressions with additional controls for GAEZ agro-climatic constraints included. These constraints are: dummies for soil nutrient availability, dummies for soil nutrient retention, dummies for soil workability, dummies for slope classes, dummies for annual precipitation/evapotranspiration, dummies for whether length of growing period is 365 days or less. All regressions include state fixed effects, a constant, and controls for the district urbanization rate, log density of district nighttime lights, log total population, log road density, share of roads of different types, distance to nearest city of 100,000 people, and a log slope index. Conley standard errors are reported with a window of 500km.
}
\end{table}
...
\begin{table}[!htb]
\begin{center}
\caption{Estimates of $\beta$ with Conley standard errors at different distances}
\label{TAB_conley}
{\footnotesize
\begin{tabularx}{\textwidth}{lXXXXXX}
\midrule
\multicolumn{7}{c}{Dependent variable is $A^{GAEZ}_{isg}$:} \\ \\
 & \multicolumn{2}{c}{Baseline (500km):} & \multicolumn{2}{c}{1000km} & \multicolumn{2}{c}{2000km} \\ \cmidrule(lr){2-3} \cmidrule(lr){4-5} \cmidrule(lr){6-7}
 & Temperate & Tropical & Temperate & Tropical & Temperate & Tropical \\
 & (1) & (2) & (3) & (4) & (5) & (6)  \\
\midrule
\input{tab_beta_crop_conley.tex}
\midrule
\end{tabularx}
}
\end{center}
\vspace{-.5cm}\singlespacing {\footnotesize Set of regressions where distance of spatial autocorrelation is set to different values. All regressions include state fixed effects, a constant, and controls for the district urbanization rate, log density of district nighttime lights, log total population, log road density, share of roads of different types, distance to nearest city of 100,000 people, and a log slope index. Conley standard errors are reported with a window of 500km.
}
\end{table}
